...
function VideoPlayer({ video, onBack, isDarkMode, user, setUser, videoList = [], playlistVideos = [], onSelectVideo, onSearchSubmit }) {
  ...
  // --- 4. TOGGLE CONTROLS LOGIC (RETAINED FROM YOUR CODE) ---
  ...
  const handleDownload = async (url) => {
    ...
    setShowDownloadMenu(false); // ✅ CLOSE MENU ON SELECTION
    try {
      ...
      for (let i = 0; i < segments.length; i++) {
        ...
        setDownloadProgress(Math.round(((i + 1) / segments.length) * 100)); // ✅ PROGRESS UPDATE
      }
  ...
  return (
    <div key={videoId} className="min-h-screen bg-[#0f0f0f] text-white p-2 md:p-6 transition-colors duration-500">
          ...
          <div className="mt-6">
              ...
              <div className="flex items-center gap-3 bg-slate-800/50 p-2 rounded-2xl border border-red-500/20">
                ...
                <div className="relative">
                  <button onClick={(e) => { e.stopPropagation(); setShowDownloadMenu(!showDownloadMenu); }} className="px-4 py-2 hover:bg-white/5 rounded-xl text-sm flex items-center gap-2">
                    {isDownloading ? <span className="animate-pulse text-red-400">Wait {downloadProgress}%</span> : <><Download size={18}/> Download</>}
                  </button>
                  {showDownloadMenu && (
            ...
            <div className="mt-4 p-4 bg-slate-800/40 rounded-xl border border-white/5">
               ...
               <div className="flex flex-wrap gap-2 mt-4">
                  {currentVideo.tags?.map((tag, i) => (
                    <button key={i} onClick={() => onSearchSubmit([], tag)} className="text-red-400 text-[11px] font-bold px-3 py-1 bg-red-500/10 rounded-full border border-red-500/20 hover:bg-red-500/20 transition-all">#{tag}</button>
                  ))}
               </div>
            </div>
            ...
            {showSummary && (
               <div className="mt-4 p-4 rounded-2xl bg-blue-900/20 border border-blue-500/30">
                  ...
                  {summaryStatus === "loading" ? <p className="animate-pulse">Generating...</p> : <p className="text-gray-200 text-sm leading-relaxed">{summary || "Summary not available."}</p>}
               </div>
            )}
          </div>
          
          {/* COMMENTS (FIXED REPLIES LOGIC) */}
      ...
      {/* PLAYLIST MODAL (FULLY PRESERVED) */}
      {showPlaylist && (
