...
function VideoPlayer({ video, onBack, isDarkMode, user, setUser, videoList = [], playlistVideos = [], onSelectVideo, onSearchSubmit }) {
  ...
  // --- LOAD VIDEO METADATA, COMMENTS, PLAYLISTS ---
  useEffect(() => {
    if (!videoId) return;
      ...
      if (!offline) {
        ...
        if (Hls.isSupported()) {
          ...
          hls.on(Hls.Events.MANIFEST_PARSED, () => {
            const resumeTime = savedTime > 0 ? savedTime : (currentVideo.watchedTime || 0);
  ...
  // --- SET QUEUE FOR PLAYLIST CONTINUITY ---
  useEffect(() => {
  ...
  // --- PROGRESS SAVING WITH 20-SECOND RESET AT END ---
  const saveProgress = async (time) => {
    ...
    // Reset to 0 when within 20 seconds of completion
  ...
  const handleAddComment = async (e) => {
    ...
    if (!user) return alert("Please sign in!");
    try {
      const res = await axios.post(`${import.meta.env.VITE_API_URL}/api/v1/comments/${videoId}`, 
        { content: commentText }, 
        { withCredentials: true }
      );
  ...
  const handleReply = async (id) => {
    ...
    try {
      ...
      setComments(prev => prev.map(c => 
        c._id === id ? { ...c, replies: [...(c.replies || []), res.data.data] } : c
      ));
  ...
  const handleCreatePlaylist = async () => {
    ...
    try {
      const res = await axios.post(`${import.meta.env.VITE_API_URL}/api/v1/playlists`, 
      ...
      if (newPlaylist && (newPlaylist._id || newPlaylist.id)) {
        ...
        await axios.post(`${import.meta.env.VITE_API_URL}/api/v1/playlists/${pid}/add`, 
          { videoId }, 
          { withCredentials: true }
        );
    ...
    <div key={videoId} className="min-h-screen bg-[#0f0f0f] text-white transition-colors duration-500">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-8">
            ...
            >
              ...
              <div className={`absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent flex flex-col justify-between p-4 transition-opacity duration-300 ${showControls || !isPlaying ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
                ...
                <div className="flex items-center justify-center gap-10">
                  <button onClick={(e) => skip(-10, e)} className="text-white hover:text-red-400 transition-colors">
                    <RotateCcw size={40} />
                  </button>
              ...
              {showSettings && (
                <div onClick={(e) => e.stopPropagation()} className="absolute right-4 top-14 w-52 bg-slate-800/95 backdrop-blur-xl rounded-2xl p-2 shadow-2xl border border-red-500/20 z-50">
                  ...
                  <button onClick={() => { setCurrentResolution("360p"); setShowSettings(false); }} className={`w-full flex justify-between items-center px-3 py-2 rounded-xl text-sm transition ${currentResolution === "360p" ? "bg-gradient-to-r from-red-600 to-red-700 text-white" : "hover:bg-red-500/20 text-zinc-300"}`}>
                    360p {currentResolution === "360p" && <Check size={14} />}
                  </button>
                  <button 
                    disabled={!currentVideo.videoUrl480}
                    onClick={() => { if (currentVideo.videoUrl480) { setCurrentResolution("480p"); setShowSettings(false); } }}
                  ...
                  {[0.5, 1, 1.5, 2].map(speed => (
                    <button key={speed} onClick={() => changeSpeed(speed)} className={`w-full flex justify-between items-center px-3 py-2 rounded-xl text-sm transition ${playbackSpeed === speed ? "bg-gradient-to-r from-red-600 to-red-700 text-white" : "hover:bg-red-500/20 text-zinc-300"}`}>
            ...
            <div className="mt-6">
              ...
              <div className="flex flex-wrap items-center justify-between gap-4 mt-4 border-b border-white/10 pb-6">
                ...
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 rounded-full bg-gradient-to-br from-red-600 to-red-700 flex items-center justify-center font-bold text-white border-2 border-red-400/30 overflow-hidden shrink-0">
                  ...
                  <div>
                    <div className="font-bold flex items-center gap-2 text-white">
                      {currentVideo.owner?.username} <CheckCircle2 size={16} className="text-red-400" />
                    </div>
                ...
                <div className="flex items-center gap-3 bg-slate-800/50 backdrop-blur-sm p-2 rounded-2xl border border-red-500/20">
                  ...
                  <div className="relative">
                    ...
                    {showDownloadMenu && (
                      <div className="absolute right-0 top-12 bg-slate-800/95 backdrop-blur-sm rounded-xl p-2 w-32 shadow-xl border border-red-500/20 z-50">
                        <button onClick={() => handleDownload(currentVideo.videoUrl)} className="block w-full px-4 py-2 hover:bg-red-500/20 rounded-lg text-left text-white transition-colors">
                          360p
                        </button>
                        <button onClick={() => handleDownload(currentVideo.videoUrl480)} className="block w-full px-4 py-2 hover:bg-red-500/20 rounded-lg text-left text-white transition-colors">
                          480p
                        </button>
                      </div>
                    )}
                  ...
                  <button onClick={() => setShowPlaylist(true)} className={`px-4 py-2 rounded-xl text-sm font-medium transition-all duration-200 ${isInPlaylist ? "bg-gradient-to-r from-red-600 to-red-700 text-white" : "hover:bg-red-500/20 text-white"}`}>
            ...
            <div className="mt-8 pb-10">
              ...
              <form onSubmit={handleAddComment} className="flex gap-4 mb-8">
                <div className="w-12 h-12 rounded-full bg-gradient-to-br from-red-600 to-red-700 flex items-center justify-center font-bold text-white border-2 border-red-400/30 shrink-0">
                  {user?.username?.charAt(0).toUpperCase() || "?"}
                </div>
                <input 
                  className="flex-1 bg-slate-800/50 border border-white/10 rounded-xl px-4 py-2 outline-none focus:border-red-500 text-white placeholder-gray-400"
                  placeholder="Add a comment..." 
                  value={commentText} 
                  onChange={e => setCommentText(e.target.value)} 
                />
                {commentText && (
                  <button type="submit" className="px-6 py-2 rounded-xl bg-gradient-to-r from-red-600 to-red-700 text-white text-sm font-bold hover:from-red-500 hover:to-red-500 transition-all">
                    Comment
                  </button>
                )}
              </form>
                  ...
                  <div key={c._id} className="flex gap-4 bg-slate-800/30 backdrop-blur-sm p-4 rounded-2xl border border-red-500/10">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-4 mt-3 text-xs opacity-70">
                        <button onClick={() => handleLikeComment(c._id)} className={`flex items-center gap-1 hover:text-red-400 ${c.likes?.includes(user?._id) ? "text-red-400" : ""}`}>
                          <ThumbsUp size={14}/> {c.likes?.length || 0}
                        </button>
                        <button onClick={() => handleDislikeComment(c._id)} className={`flex items-center gap-1 hover:text-red-400 ${c.dislikes?.includes(user?._id) ? "text-red-400" : ""}`}>
                          <ThumbsDown size={14}/> {c.dislikes?.length || 0}
                        </button>
                        <button onClick={() => setShowReplyBox(p => ({...p, [c._id]: !p[c._id]}))} className="hover:text-red-400">
                          Reply
                        </button>
                        {c.replies?.length > 0 && (
                          <button onClick={() => setShowReplies(p => ({...p, [c._id]: !p[c._id]}))} className="hover:text-red-400">
                            {showReplies[c._id] ? '▲ Hide' : `▼ View ${c.replies.length} replies`}
                          </button>
                        )}
                      </div>
                      ...
                      {showReplyBox[c._id] && (
                        <div className="mt-3 flex gap-2">
                          <input 
                            className="flex-1 bg-black/30 border-b border-white/20 p-2 outline-none text-sm text-white"
                            placeholder="Write a reply..." 
                            value={replyText[c._id] || ""} 
                            onChange={e => setReplyText(p => ({...p, [c._id]: e.target.value}))} 
                          />
                          <button onClick={() => handleReply(c._id)} className="text-red-400 text-sm font-bold">
                            Post
                          </button>
                        </div>
                      )}
          ...
          <div className="lg:col-span-4">
            ...
            <div className="flex flex-col gap-4">
              {(relatedVideos.length ? relatedVideos : videoList)
                .filter(v => (v._id || v.id) !== videoId)
                .slice(0, 10)
                .map((v) => (
                  <div 
                    key={v._id || v.id} 
      ...
      {showPlaylist && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div onClick={(e) => e.stopPropagation()} className="bg-slate-800/95 backdrop-blur-xl p-6 rounded-3xl w-full max-w-md border border-red-500/20 shadow-2xl">
            <h2 className="text-xl font-bold mb-6 flex items-center gap-2 text-white">
              <Clock className="text-red-500"/> Save to Playlist
            </h2>
            ...
            <div className="space-y-3 max-h-60 overflow-y-auto mb-6 pr-2">
              {playlists.map(p => (
                <button 
                  key={p._id} 
                  className="w-full text-left p-4 hover:bg-red-500/20 bg-white/5 rounded-2xl border border-white/5 text-white transition-all duration-200"
                  onClick={async () => { 
                    await axios.post(`${import.meta.env.VITE_API_URL}/api/v1/playlists/${p._id}/add`, 
                      { videoId }, 
                      { withCredentials: true }
                    ); 
                    setIsInPlaylist(true); 
            ...
            <input 
              className="w-full bg-white/5 border border-white/10 p-4 rounded-2xl outline-none focus:border-red-500 mb-4 text-white placeholder-gray-400"
            ...
            <button 
              className="w-full bg-red-600 hover:bg-red-500 p-4 rounded-2xl font-bold text-white transition-all duration-200"
            ...
            <button 
              ...
              className="w-full text-center mt-4 text-gray-500 hover:text-white transition-colors"
            >
              Cancel
            </button>
          </div>
